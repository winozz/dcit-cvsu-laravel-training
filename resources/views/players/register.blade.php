<x-app title="Create Account">
    <div class="surface max-w-md mx-auto space-y-4">
        ...
        <form method="POST" action="{{ route('players.register') }}" class="space-y-3">
            ...
            <input type="text" name="username" value="{{ old('username') }}" required minlength="3" maxlength="32" pattern="[A-Za-z0-9]+" class="w-full rounded bg-[#0f142a] border border-[#2f3a66] p-2" placeholder="e.g. wordsmith" />

            <label class="block text-sm font-semibold">Email</label>
            <input type="email" name="email" value="{{ old('email') }}" required maxlength="255" class="w-full rounded bg-[#0f142a] border border-[#2f3a66] p-2" placeholder="e.g. you@example.com" />
